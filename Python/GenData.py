...
###################################################################################################
def main():
    imgTrainingNumbers = cv2.imread("Photos/GenPhotos/Gen_scorbox_only.jpg")
    if imgTrainingNumbers is None:
        print ("Gen data: error: image not read from file... :(")
        # os.system("pause")
        return
    # end if
    ...
    imgContours, npaContours, npaHierarchy = cv2.findContours(imgThreshCopy,        # input image, make sure to use a copy since the function will modify this image in the course of finding contours
                                                 cv2.RETR_TREE,                 # retrieve the outermost contours only  RETR_EXTERNAL
    ...
    np.savetxt("OCR_file/classifications.txt", npaClassifications)           # write flattened images to file
    np.savetxt("OCR_file/flattened_images.txt", npaFlattenedImages)          #כנל:)
